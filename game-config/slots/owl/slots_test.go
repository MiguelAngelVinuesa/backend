...
func TestNew(t *testing.T) {
	testCases := []struct {
		...
		{name: "RTP 92", rtp: 92, min: 2.0, max: 2000.0},
		{name: "RTP 93", rtp: 93, fail: true},
		{name: "RTP 94", rtp: 94, min: 2.0, max: 2000.0},
		{name: "RTP 95", rtp: 95, fail: true},
		{name: "RTP 96", rtp: 96, min: 2.0, max: 2000.0},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.fail {
				...
				r := analysis.AcquireRounds(tg.OWLnr, "p1", 10000000, reels, rows, false, g.MaxPayout(), AllSymbols(), AllActions(tc.rtp), Paylines(), Flags())
				require.NotNil(t, r)
				defer r.Release()
