...
func TestNew(t *testing.T) {
	testCases := []struct {
		...
		{name: "RTP 92", rtp: 92, actions: actions92all, min: 50.0, max: 200.0},
		{name: "RTP 93", rtp: 93, fail: true},
		{name: "RTP 94", rtp: 94, actions: actions94all, min: 50.0, max: 200.0},
		{name: "RTP 95", rtp: 95, fail: true},
		{name: "RTP 96", rtp: 96, actions: actions96all, min: 50.0, max: 200.0},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.fail {
				...
				r := analysis.AcquireRounds(tg.BOTnr, "p1", 10000000, reels, rows, false, g.MaxPayout(), symbols2, tc.actions, Paylines(), Flags())
				require.NotNil(t, r)
				defer r.Release()
...
func BenchmarkAnalyze96(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		a := analysis.AcquireRounds(tg.BOTnr, "x", 1000000, 5, 3, false, g.MaxPayout(), symbols2, actions96all, paylines, flags)
		for iy := 0; iy < 1000; iy++ {
			res := g.Round(0)
...
func BenchmarkRound91Logged(b *testing.B) {
	g := NewLogged(92)
	defer g.Release()
