...
func (d *decoder) parsePLTE(length uint32) error {
	...
	case cbP1, cbP2, cbP4, cbP8:
		...
		for i := 0; i < np; i++ {
			d.palette[i] = color.RGBA{R: d.tmp[3*i+0], G: d.tmp[3*i+1], B: d.tmp[3*i+2], A: 0xff}
		}
		for i := np; i < 256; i++ {
			d.palette[i] = color.RGBA{A: 0xff}
		}
...
func (d *decoder) parsetRNS(length uint32) error {
	...
	case cbP1, cbP2, cbP4, cbP8:
		...
		for i := 0; i < n; i++ {
			...
			d.palette[i] = color.NRGBA{R: rgba.R, G: rgba.G, B: rgba.B, A: d.tmp[i]}
		}
...
func (d *decoder) readImagePass(r io.Reader, pass int, allocateOnly bool) (image.Image, error) {
	...
	for y := 0; y < height; y++ {
			...
			if d.useTransparent {
				...
				for x := 0; x < width; x += 8 {
					...
					for x2 := 0; x2 < 8 && x+x2 < width; x2++ {
						...
						nrgba.SetNRGBA(x+x2, y, color.NRGBA{R: ycol, G: ycol, B: ycol, A: acol})
						b <<= 1
					}
				}
			} else {
				for x := 0; x < width; x += 8 {
					b := cdat[x/8]
					for x2 := 0; x2 < 8 && x+x2 < width; x2++ {
						gray.SetGray(x+x2, y, color.Gray{Y: (b >> 7) * 0xff})
						b <<= 1
					}
				}
			}
		case cbG2:
			if d.useTransparent {
				...
				for x := 0; x < width; x += 4 {
					b := cdat[x/4]
					for x2 := 0; x2 < 4 && x+x2 < width; x2++ {
						gray.SetGray(x+x2, y, color.Gray{Y: (b >> 6) * 0x55})
						b <<= 2
					}
				}
			}
		case cbG4:
			if d.useTransparent {
				...
				for x := 0; x < width; x += 2 {
					b := cdat[x/2]
					for x2 := 0; x2 < 2 && x+x2 < width; x2++ {
						gray.SetGray(x+x2, y, color.Gray{Y: (b >> 4) * 0x11})
						b <<= 4
					}
				}
			}
		case cbG8:
			if d.useTransparent {
				...
				for x := 0; x < width; x++ {
					...
					nrgba.SetNRGBA(x, y, color.NRGBA{R: ycol, G: ycol, B: ycol, A: acol})
				}
			} else {
		...
		case cbGA8:
			for x := 0; x < width; x++ {
				ycol := cdat[2*x+0]
				nrgba.SetNRGBA(x, y, color.NRGBA{R: ycol, G: ycol, B: ycol, A: cdat[2*x+1]})
			}
		case cbTC8:
			if d.useTransparent {
		...
		case cbG16:
			if d.useTransparent {
				...
				for x := 0; x < width; x++ {
					...
					nrgba64.SetNRGBA64(x, y, color.NRGBA64{R: ycol, G: ycol, B: ycol, A: acol})
				}
			} else {
				for x := 0; x < width; x++ {
					ycol := uint16(cdat[2*x+0])<<8 | uint16(cdat[2*x+1])
					gray16.SetGray16(x, y, color.Gray16{Y: ycol})
				}
			}
		case cbGA16:
			for x := 0; x < width; x++ {
		...
		case cbTC16:
			if d.useTransparent {
				...
				for x := 0; x < width; x++ {
					...
					nrgba64.SetNRGBA64(x, y, color.NRGBA64{R: rcol, G: gcol, B: bcol, A: acol})
				}
			} else {
				for x := 0; x < width; x++ {
					rcol := uint16(cdat[6*x+0])<<8 | uint16(cdat[6*x+1])
					gcol := uint16(cdat[6*x+2])<<8 | uint16(cdat[6*x+3])
					bcol := uint16(cdat[6*x+4])<<8 | uint16(cdat[6*x+5])
					rgba64.SetRGBA64(x, y, color.RGBA64{R: rcol, G: gcol, B: bcol, A: 0xffff})
				}
			}
		case cbTCA16:
			for x := 0; x < width; x++ {
